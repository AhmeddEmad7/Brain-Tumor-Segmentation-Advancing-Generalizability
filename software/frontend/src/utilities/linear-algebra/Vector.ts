...
class Vector {
    ...
    isZero() {
        return this.components.reduce(
            (acc, _, index) => acc && MathUtils.isNearlyZero(this.components[index]),
            true
        );
    }

    abs() {
        return new Vector(...this.components.map((component) => Math.abs(component)));
    }

    round() {
        return new Vector(...this.components.map((component) => Math.round(component)));
    }
