...
const ReportingTab = () => {
    const { selectedStudyReports, currentStudyInstanceUid,
        renderingEngineId, selectedViewportId } = useSelector((store: IStore) => store.viewer);
    ...
    const captureSnapshotFromElement = (viewport: Types.IViewport) => {
      return new Promise<string>((resolve, reject) => {
        ...
        ctx.drawImage(
          mainCanvas,
          0, 0, mainCanvas.width, mainCanvas.height, // source: intrinsic dimensions
          0, 0, displayWidth, displayHeight           // destination: display dimensions
        );
        ...
        return (
            ...
            {snapshots.map((snapshot) => (
              <div key={snapshot.index} style={{ margin: '5px' }}>
                ...
                onClick={() => handleDeleteSnapshot(snapshot.index)} >Delete</DeleteIcon>
              </div>
            ))}
          </div>
        );
