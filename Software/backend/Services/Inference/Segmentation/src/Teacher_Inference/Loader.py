...
def load_sequences_from_paths(t1c_path, t1n_path, t2f_path, t2w_path):   # lazem yekon el path .nii aw .nii.gz     
        ...
        processed_data_with_norm = preprocess_data(raw_data, normalize=True)
        norm_imgs  = np.array(processed_data_with_norm['imgs'])
        float_volumes = { 'imgs'  : torch.from_numpy( norm_imgs ).type( torch.FloatTensor )}

        processed_data = preprocess_data(raw_data, normalize=False)
        imgs  = np.array(processed_data['imgs'])
        int_volumes = { 'imgs'  : torch.from_numpy( imgs ).type( torch.IntTensor )}
